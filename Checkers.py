...
class Checkers:
    ...
    def print_board(self):
        ...
        console."""
        #print("wasted time:",self.wasted_time,"seconds")
    ...
    def predict(self, state):
        ...
        """
        nn_inp = np.moveaxis(state[:14],0,-1)
        nn_inp = nn_inp.reshape(1,8,8,14)
        prob_planes, q_value = self.neural_net.predict(nn_inp)
        prob_planes, q_value = prob_planes[0].reshape((8,8,8)), q_value[0][0] 
        action_mask = state[6:14]
        prob_planes *= action_mask
